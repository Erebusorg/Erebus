...
///
/// Byte for byte what `FeePool._payoutHash` builds, including the chain id, the
/// pool address, and the deadline, so a proof cannot be lifted to another chain,
/// another deployment, another set of recipients, or a later time than the payer
/// meant it for.
pub fn payout_hash(
    chain_id: u64,
    pool: [u8; 20],
    ...
    data.extend_from_slice(&u256_be(deadline as u128));
...
///
/// @notice Generated from the repo root by `cargo run --release --manifest-path
/// mixnet/Cargo.toml -p erebus-fees -- export-verifier`
...
mod tests {
    ...
    fn a_payout_binds_the_chain_the_pool_and_the_split() {
        ...
        let base = payout_hash(1, pool, 99, &nodes, &amounts).unwrap();
        assert_ne!(base, payout_hash(2, pool, 99, &nodes, &amounts).unwrap());
        assert_ne!(
            base,
            payout_hash(1, [9u8; 20], 99, &nodes, &amounts).unwrap()
        );
        assert_ne!(base, payout_hash(1, pool, 100, &nodes, &amounts).unwrap());
        assert_ne!(
            base,
            payout_hash(1, pool, 99, &[[2u8; 20], [4u8; 20]], &amounts).unwrap()
        );
        assert_ne!(
            base,
            payout_hash(1, pool, 99, &nodes, &[6u128, 4u128]).unwrap()
        );
    }

    #[test]
    fn a_payout_needs_matching_arrays() {
        assert!(payout_hash(1, [0u8; 20], 0, &[], &[]).is_err());
        assert!(payout_hash(1, [0u8; 20], 0, &[[1u8; 20]], &[1u128, 2u128]).is_err());
    }
